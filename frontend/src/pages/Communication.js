...
const Communication = () => {
    ...
    useEffect(() => {
        ...
        socket.on("new_message", (msg) => {
            if (selectedUserRef.current?._id === msg.sender) {
                ...
                setRecipients((prev) => prev.map(u => 
                    u._id === msg.sender ? { ...u, unreadCount: (u.unreadCount || 0) + 1 } : u
                ));
            }
        });
    ...
    const handleSelect = async (user) => {
        ...
        try {
            ...
            if (user.unreadCount > 0) {
                ...
                setRecipients(prev => prev.map(u => u._id === user._id ? { ...u, unreadCount: 0 } : u));
            }
    ...
    return (
        <div className="comm-page">
            <div className={`comm-glass-container ${isMobileChatOpen ? "mobile-chat-active" : ""}`}>
                ...
                <aside className="comm-sidebar">
                    ...
                    <div className="contact-list-container">
                        {recipients.map(u => (
                            <div key={u._id} className={`contact-row-item ${selectedUser?._id === u._id ? 'selected' : ''}`} onClick={() => handleSelect(u)}>
                                <div className={`avatar-box ${u.role === 'admin' ? 'admin-box' : ''}`}>
                                        ...
                                        {u.sites && u.sites.length > 0 && (
                                            <div className="site-tag-container">
                                                {u.sites.map((site, index) => (
                                                    <span key={index} className="site-pill">{site}</span>
                                                ))}
                                            </div>
                                        )}
                                    </div>
                    ...
                    {selectedUser ? (
                        <>
                            <header className="chat-header-bar">
                                <button className="comm-back-btn" onClick={() => setIsMobileChatOpen(false)}>←</button>
                                <div className="header-info">
                                    <h3>{selectedUser.name}</h3>
                                    <div className="header-sub-info">
                                        <span className="header-status">{selectedUser.role}</span>
                                        {selectedUser.sites?.map((s, i) => (
                                            <span key={i} className="header-site-pill">{s}</span>
                                        ))}
                                    </div>
                                </div>
                            </header>
                            ...
                            <div className="chat-messages-area">
                                {messages.map(m => (
                                    <div key={m._id} className={`comm-bubble ${m.sender === selectedUser._id ? 'received' : 'sent'}`}>
                                        {m.fileUrl && (
                                            <div className="attachment-content">
                                                {m.fileType === "image" ? (
                                                    <img 
                                                        ...
                                                        onClick={() => window.open(`http://localhost:5000${m.fileUrl}`)} 
                                                    />
                                                ) : (
                                                    <a href={`http://localhost:5000${m.fileUrl}`} target="_blank" rel="noreferrer" className="comm-file-link">
                            ...
                            {file && (
                                <div className="file-selection-preview">
                                    <span>📎 {file.name}</span>
                                    <button className="remove-file-btn" onClick={() => {setFile(null); fileInputRef.current.value="";}}>✕</button>
                                </div>
                            )}

                            <form className="comm-input-bar" onSubmit={handleSend}>
                                <button type="button" className="comm-attach-btn" onClick={() => fileInputRef.current.click()}>📎</button>
                                <input type="file" ref={fileInputRef} hidden onChange={(e) => setFile(e.target.files[0])} accept="image/*,.pdf" />
                                <input type="text" value={text} onChange={e => setText(e.target.value)} placeholder="Type a message..." />
                                <button type="submit" disabled={!text.trim() && !file}>➤</button>
                            </form>
                        </>
                    ) : (
